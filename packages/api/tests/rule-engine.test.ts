...
describe('RuleEngine', () => {
  ...
  describe('evaluate — COST_CAP_SESSION', () => {
    it('returns PAUSE violation when session cost exceeds cap', async () => {
      ruleEngine._setRulesForTest(TEST_ORG, [
        { id: 'r1', orgId: TEST_ORG, name: 'Session cap', type: 'COST_CAP_SESSION', scope: { global: true }, condition: { maxCost: 50 }, action: 'PAUSE', enabled: true },
      ]);
  ...
  describe('evaluate — BURN_RATE_LIMIT', () => {
    it('returns ALERT on first burn rate violation', async () => {
      ruleEngine._setRulesForTest(TEST_ORG, [
        { id: 'r3', orgId: TEST_ORG, name: 'Rate limit', type: 'BURN_RATE_LIMIT', scope: { global: true }, condition: { maxRate: 10000 }, action: 'ALERT', enabled: true },
      ]);
  ...
  describe('evaluate — SESSION_DURATION', () => {
    it('returns PAUSE when session exceeds max duration', async () => {
      ruleEngine._setRulesForTest(TEST_ORG, [
        { id: 'r4', orgId: TEST_ORG, name: 'Max 60 min', type: 'SESSION_DURATION', scope: { global: true }, condition: { maxMinutes: 60 }, action: 'PAUSE', enabled: true },
      ]);
  ...
  describe('scope matching', () => {
    it('global scope matches any session', async () => {
      ruleEngine._setRulesForTest(TEST_ORG, [
        { id: 'r1', orgId: TEST_ORG, name: 'Global cap', type: 'COST_CAP_SESSION', scope: { global: true }, condition: { maxCost: 10 }, action: 'ALERT', enabled: true },
      ]);
    ...
    it('sessionType scope filters correctly', async () => {
      ruleEngine._setRulesForTest(TEST_ORG, [
        { id: 'r1', orgId: TEST_ORG, name: 'Agent cap', type: 'COST_CAP_SESSION', scope: { sessionType: 'agent_local' }, condition: { maxCost: 10 }, action: 'ALERT', enabled: true },
      ]);
  ...
  describe('evaluate — COST_CAP_DAILY', () => {
    it('detects violation using Redis cached value', async () => {
      ruleEngine._setRulesForTest(TEST_ORG, [
        { id: 'r5', orgId: TEST_ORG, name: 'Daily cap', type: 'COST_CAP_DAILY', scope: { global: true }, condition: { maxCost: 200 }, action: 'ALERT', enabled: true },
      ]);
      ...
      expect(mockRedis.get).toHaveBeenCalledWith(`pulse:daily_cost:${TEST_ORG}`);
    });

    it('falls back to DB when Redis cache misses', async () => {
      ruleEngine._setRulesForTest(TEST_ORG, [
        { id: 'r5', orgId: TEST_ORG, name: 'Daily cap', type: 'COST_CAP_DAILY', scope: { global: true }, condition: { maxCost: 100 }, action: 'ALERT', enabled: true },
      ]);
